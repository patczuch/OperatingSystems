...
int main(int argc, char **argv)
{
	...
	message msg;
	msg.mtype = INIT;
	sendto(socket1, (const char *)&msg, sizeof(int), 0, (struct sockaddr*) &ser, sizeof ser);
	
	while (recvfrom(socket1, &msg, MESSAGE_MAX, 0, NULL, NULL) == -1);
	...
	while (run)
	{
		buf_size = recvfrom(socket1, (char *)&msg, MESSAGE_MAX, MSG_DONTWAIT, NULL, NULL);
		if (buf_size != -1)
		{
			...
			switch(msg.mtype)
			{
				...
				case REFRESH:
					...
					sendto(socket1, (const char *)&msg, sizeof(int) * 2, 0, (struct sockaddr*) &ser, sizeof ser);
					break;
				default:
					break;
			}
		...
		if (input_available())
		{
			...
			else if (strcmp(c, "2ALL") == 0)
			{
				...
				sendto(socket1, (const char *)&msg, sizeof(int) * 2 + strlen(c) + 1, 0, (struct sockaddr*) &ser, sizeof ser);
			}
			else if (strcmp(c, "2ONE") == 0)
			{
				...
				sendto(socket1, (const char *)&msg, sizeof(int) * 3 + strlen(c) + 1, 0, (struct sockaddr*) &ser, sizeof ser);
			}
		}
	}
